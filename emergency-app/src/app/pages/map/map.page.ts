...
export class MapPage implements ViewWillEnter {
  ...
  filterAlerts() {
    ...
    const filteredAlerts = this.activeAlerts.filter((alert) => {
      ...
      const isActive =
        now.getTime() - alertTime.getTime() < 24 * 60 * 60 * 1000;
      const alertCategory = (alert.category || '')
        .toLowerCase()
        .replace(/\s+/g, '-');
      // Filter if they match any type of alert
      const matchesType =
        this.selectedType === 'all' || alertCategory === this.selectedType;
      // Filtes if it matches any status of alert based on current time
      const matchesStatus =
        this.selectedStatus === 'all' ||
        (this.selectedStatus === 'active' ? isActive : !isActive);
      // Fitler if it matches sefverity  levels on alerts
      const matchesSeverity =
        this.selectedSeverityFilter === 'all' ||
        (alert.severity && String(alert.severity).toLowerCase() === this.selectedSeverityFilter.toLowerCase());
      // Return all alerts that match the type, status and severity filters
